...
ssh www-data@opendata02.nine.ch "mysqldump -u nmd_opendata_ch $databasePassword nmd_opendata_ch | gzip -c1" | gunzip -c | sed -e 's/USING BTREE//' | docker-compose run --rm mysql /usr/bin/mysql -h mysql -u root opendata

docker-compose run --rm mysql /usr/bin/mysql -h mysql -u root opendata --execute=" \
    UPDATE wp_site SET domain = 'localhost:8000'; \
    UPDATE wp_blogs SET domain = 'localhost:8000' where domain = 'opendata.ch'; \
    UPDATE wp_2_options SET option_value = 'http://localhost:8000/wordpress' WHERE option_name = 'siteurl'; \
    UPDATE wp_2_options SET option_value = 'http://localhost:8000/' WHERE option_name = 'home'; \
